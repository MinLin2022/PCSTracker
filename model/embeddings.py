...
def get_3d_embedding(xyz, C, cat_coords=True):
    ...
    B, N, D = xyz.shape
    assert D == 3, "input must be 3D point coordinates (x, y, z)"
    ...
    def encode(coord):
        pe = torch.zeros(B, N, C, device=xyz.device, dtype=torch.float32)
        pe[:, :, 0::2] = torch.sin(coord * div_term)
        pe[:, :, 1::2] = torch.cos(coord * div_term)
        return pe
    ...
    if cat_coords:
        pe = torch.cat([xyz, xyz,pe], dim=2)  # B, N, 3+3 + C*3
